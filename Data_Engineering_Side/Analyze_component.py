...
df["extracted_skills"] = df["job_description"].apply(extract_skills)
'''
# df["job_description"] → selects the column containing job descriptions.
# .apply(extract_skills) → runs the function extract_skills() on each row of that column.
# The results are stored in a new column called extracted_skills.
############## 
...
skill_counts = all_skills.value_counts()
'''
# df["extracted_skills"] → column of lists
# .explode() → turns each list into separate rows.
# Example: [PLC, Python] becomes two rows: PLC, Python.
# value_counts() → counts how many times each skill appears.

############## 
r'''
print(df) # shows the DataFrame with job descriptions + extracted skills.
print(skill_counts) # shows frequency of skills across all job descriptions.
'''
